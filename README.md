...
* `--cache`: Optional. Some miners extract data from the client cache folder. To enable those, pass a path to the cache, e.g. `C:\users\<user>\AppData\Local\CCP\EVE\c_ccp_eve_online_tq_tranquility\cache`.
