...
class MachoNetCallsMiner(MachoNetBase):
    ...
    def _get_container_name(self, entity_name):
        ...
        svc_args_line = ', '.join(self._as_text(a) for a in svc_args)
        call_args_line = ', '.join(self._as_text(a) for a in call_args)
        return '{}({})_{}({})'.format(
            self._as_text(svc_name), svc_args_line, self._as_text(call_name), call_args_line)
